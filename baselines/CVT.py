"""
Based on the implementation of: https://github.com/microsoft/CvT
"""
...
class Attention(nn.Module):
    ...
    def __init__(self, cls_token, in_dim, num_heads, kernel_size, stride_q, stride_kv, padding_q, padding_kv, attn_drop_p, attn_proj_drop_p):
        ...
        self.conv_proj_q = nn.Sequential(nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=kernel_size,
                                                   stride=stride_q, padding=padding_q, groups=in_dim),
                                         nn.BatchNorm2d(num_features=in_dim),
                                         nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=1),
                                         )
        self.conv_proj_k = nn.Sequential(nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=kernel_size,
                                                   stride=stride_kv, padding=padding_kv, groups=in_dim),
                                         nn.BatchNorm2d(num_features=in_dim),
                                         nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=1),
                                         )
        self.conv_proj_v = nn.Sequential(nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=kernel_size,
                                                   stride=stride_kv, padding=padding_kv, groups=in_dim),
                                         nn.BatchNorm2d(num_features=in_dim),
                                         nn.Conv2d(in_channels=in_dim, out_channels=in_dim, kernel_size=1),
                                         )
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, cls_token, n_layers, emb_dim, mlp_expansion, num_heads, kernel_size, stride_q, stride_kv,
                 padding_q, padding_kv, attn_drop_p, attn_proj_drop_p, drop_path_p, drop_p):
        super().__init__()
...
class ConvolutionalVisionTransformer(nn.Module):
    """
        Implementation of the Convolutional Vision Transformer (CvT).
        ...
        Args:
            ...
            drop_p (float): Dropout probability for MLP layers.
        """
    def __init__(self, in_channel=3, num_classes=1000, emb_dim=(64, 128, 192), n_layers=(1, 2, 6),
                 kernel_size=(7, 3, 3), stride=(4, 2, 2), padding=(2, 1, 1), mlp_expansion=4, num_heads=(1, 3, 6),
                 attn_kernel_size=3, stride_q=(1, 1, 1), stride_kv=(1, 1, 1), padding_q=(1, 1, 1), padding_kv=(1, 1, 1),
                 attn_drop_p=0.1, attn_proj_drop_p=0.1, drop_path_p=0.1, drop_p=0.1):
        super().__init__()
